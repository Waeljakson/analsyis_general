(() => {
  ...
  async function createCustomEvent(payload){
    if(!state.access||!state.user||!state.session)throw new Error('all_access_required');
  ...
  async function deleteCustomEvent(id){if(!state.session)return;await request(`/rest/v1/premium_calendar_events?id=eq.${encodeURIComponent(id)}`,{method:'DELETE',token:state.session.access_token,prefer:'return=minimal'});state.customEvents=state.customEvents.filter(x=>x.id!==id);derive()}
