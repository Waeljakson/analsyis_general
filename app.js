...
const state = {mode: null, files: [], result: null, user: null, account: null, profile: null, archive: [], pendingSchoolLogo: null, adminRequests: [], adminUsers: [], securityTimer: null, supportThread: null, supportMessages: [], supportPollTimer: null, adminSupportThreads: [], adminSelectedThread: null, adminSupportMessages: []};
...
function isPremiumAccess() { if (!state.account || !state.account.is_active) return false; if (state.account.is_system_admin) return true; if (state.account.plan !== "premium" || !state.account.premium_expires_at) return false; return new Date(state.account.premium_expires_at).getTime() > Date.now(); }
function premiumExpiryLabel() { if (state.account?.is_system_admin) return "مدير النظام — جميع المزايا متاحة"; if (!isPremiumAccess()) return "الحفظ والطباعة والتصدير غير متاحة"; const period=state.account?.subscription_period==="monthly"?"شهري":"سنوي"; return `اشتراك ${period} — صالح حتى ${new Date(state.account.premium_expires_at).toLocaleDateString("ar-SA")}`; }
...
function updatePremiumUi() {
  ...
  if(el.currentUserPlan) { const period=state.account?.subscription_period==="monthly"?" شهري":" سنوي"; el.currentUserPlan.textContent=state.account?.is_system_admin?"مدير النظام":access?`Premium${period}`:"تجريبي"; }
  if(el.subscriptionExpiry) el.subscriptionExpiry.textContent=premiumExpiryLabel();
  if(el.requestPremiumButton){el.requestPremiumButton.hidden=access;el.requestPremiumButton.disabled=false;}
...
function requirePremium(feature="هذه الميزة") { if(isPremiumAccess()) return true; showDatabaseStatus(`${feature} متاحة للمشتركين Premium فقط. اختر 10 ريالات شهريًا أو 50 ريالًا سنويًا.`,true); document.querySelector(".account-panel")?.scrollIntoView({behavior:"smooth"}); return false; }
...
function roleLabel() { return isPremiumAccess() ? "Premium — جميع مزايا الحفظ والتصدير" : "تجريبي — التحليل والعرض فقط"; }
...
async function applySession(session) {
  state.user=session?.user||null;state.account=null;state.profile=null;
  if(state.user){try{state.account=await loadProfile(state.user);state.profile=state.account;}catch(error){console.error(error);showLoginStatus("تم تسجيل الدخول، لكن تعذر تحميل بيانات الحساب.",true);}}
  ...
  if(loggedIn){
    ...
    applyBranding();updatePremiumUi();hideDatabaseStatus();startSupportPolling();
  }else{state.archive=[];state.supportThread=null;state.supportMessages=[];state.adminSupportThreads=[];state.adminSelectedThread=null;stopSupportPolling();closeSupportChat();if(el.archivePanel)el.archivePanel.hidden=true;if(el.adminPanel)el.adminPanel.hidden=true;}
}
...
async function signOut(){await db.auth.signOut();await applySession(null);showLoginStatus("تم تسجيل الخروج.");}
...
function selectedPremiumPlan(){
  const period=el.paymentPlanInputs?.find(input=>input.checked)?.value || "yearly";
  return period==="monthly"
    ? {period:"monthly",label:"الاشتراك الشهري",amount:10,amountText:"10 ريالات",months:1,durationText:"شهر واحد"}
    : {period:"yearly",label:"الاشتراك السنوي",amount:50,amountText:"50 ريالًا",months:12,durationText:"12 شهرًا"};
}
function buildWhatsAppMessage(plan){
  ...
  return [
    ...
    `الخطة: ${plan.label} — ${plan.amountText}`,
    `الاسم: ${name}`,
    `المدرسة: ${school}`,
    `البريد: ${email}`
  ].join("\n");
...
function updatePaymentPlanUi(){
  ...
  if(el.paymentModalDescription) el.paymentModalDescription.textContent=`اختر ${plan.label}، ثم تواصل عبر واتساب للحصول على بيانات الاشتراك وبعدها أرسل طلب التفعيل.`;
  if(el.paymentConfirmText) el.paymentConfirmText.textContent=`أؤكد أنني تواصلت عبر واتساب بخصوص ${plan.label} بقيمة ${plan.amountText}، وأرغب في إرسال طلب التفعيل.`;
  if(el.paymentSecurityNote) el.paymentSecurityNote.innerHTML=`<strong>تنبيه:</strong> لن يتم تفعيل ${plan.label} إلا بعد التواصل عبر واتساب واستكمال إجراءات الاشتراك.`;
  if(el.whatsappContactButton){
    const message=encodeURIComponent(buildWhatsAppMessage(plan));
    el.whatsappContactButton.href=`https://wa.me/966582712620?text=${message}`;
  }
  el.paymentConfirmCheckbox.checked=false;
  el.confirmPremiumRequestButton.disabled=true;
}
...
async function requestPremium(){
  if(isPremiumAccess()) return;
  if(!el.paymentConfirmCheckbox?.checked){showDatabaseStatus("يجب التواصل عبر واتساب أولًا ثم تأكيد التواصل قبل إرسال طلب التفعيل.",true);return;}
  el.confirmPremiumRequestButton.disabled=true;
  const original=el.confirmPremiumRequestButton.textContent;
  el.confirmPremiumRequestButton.textContent="جارٍ إرسال الطلب...";
  try{
    const plan=selectedPremiumPlan();
    const note=[
      `طلب ${plan.label} بقيمة ${plan.amountText} للمدرسة: ${state.account?.school_name||"غير محددة"}`,
      "أكد المستخدم أنه تواصل عبر واتساب على الرقم 00966582712620 للحصول على بيانات الاشتراك.",
      "التفعيل مشروط بمراجعة التواصل عبر واتساب واستكمال إجراءات الاشتراك."
    ].join(" | ");
    const{error}=await db.rpc("premium_request_subscription_plan",{p_billing_period:plan.period,p_user_note:note});
    if(error)throw error;
    closePaymentModal();
    showDatabaseStatus("تم إرسال طلب التفعيل. لن يتم التفعيل إلا بعد التواصل عبر واتساب واستكمال إجراءات الاشتراك.");
    el.requestPremiumButton.textContent="تم إرسال طلب التفعيل";
  }catch(error){showDatabaseStatus(error.message||"تعذر إرسال الطلب.",true);}
  finally{el.confirmPremiumRequestButton.disabled=!el.paymentConfirmCheckbox?.checked;el.confirmPremiumRequestButton.textContent=original;}
}
...
async function loadAdminData(){
  ...
  const[{data:req,error:e1},{data:users,error:e2}]=await Promise.all([
    db.from("premium_subscription_requests").select("id,user_id,amount_sar,billing_period,duration_months,status,user_note,requested_at").eq("status","pending").order("requested_at",{ascending:true}),
    db.from("premium_accounts").select("user_id,full_name,email,school_name,plan,subscription_period,premium_expires_at,is_system_admin,is_active,created_at").order("created_at",{ascending:false})
  ]);
  if(e1||e2){showDatabaseStatus((e1||e2).message,true);return;}
  state.adminRequests=req||[];state.adminUsers=users||[];renderAdminData();await loadAdminSupportThreads();
}
function renderAdminData(){
  const userMap=Object.fromEntries(state.adminUsers.map(u=>[u.user_id,u]));
  el.subscriptionRequestsList.innerHTML=state.adminRequests.length?state.adminRequests.map(r=>{const u=userMap[r.user_id]||{};const monthly=r.billing_period==="monthly";const periodLabel=monthly?"شهري":"سنوي";const months=Number(r.duration_months)||(monthly?1:12);return `<article class="admin-item"><h4>${escapeHtml(u.school_name||u.full_name||"مستخدم")}</h4><p>${escapeHtml(u.email||"")}</p><p>${Number(r.amount_sar).toFixed(0)} ريال · اشتراك ${periodLabel} · ${new Date(r.requested_at).toLocaleString("ar-SA")}</p><p>${escapeHtml(r.user_note||"")}</p><div class="actions inline-actions"><button class="primary-button" data-approve-request="${r.id}" data-user-id="${r.user_id}" data-period="${r.billing_period||"yearly"}" data-months="${months}">تفعيل ${monthly?"شهر":"سنة"}</button><button class="danger-button" data-reject-request="${r.id}">رفض</button></div></article>`}).join(""):"<div class='empty-state'>لا توجد طلبات معلقة.</div>";
  el.premiumUsersList.innerHTML=state.adminUsers.length?state.adminUsers.map(u=>{const isSelf=u.user_id===state.user?.id;const plan=u.is_system_admin?"مدير النظام":u.plan==="premium"?`Premium ${u.subscription_period==="monthly"?"شهري":"سنوي"}`:"تجريبي";const roleButton=u.is_system_admin?`<button class="secondary-button compact-button" data-set-admin="false" data-user-id="${u.user_id}" ${isSelf?"disabled title='لا يمكن إلغاء صلاحيتك من حسابك الحالي'":""}>إلغاء صلاحية المدير</button>`:`<button class="primary-button compact-button" data-set-admin="true" data-user-id="${u.user_id}">تعيين مدير</button>`;return `<article class="admin-item admin-user-item"><div class="admin-user-title"><div><h4>${escapeHtml(u.school_name||u.full_name||"مستخدم")}</h4><p>${escapeHtml(u.email||"بريد غير متاح")}</p></div><span class="admin-role-badge ${u.is_system_admin?"manager":"user"}">${plan}</span></div><p>${u.premium_expires_at?`ينتهي الاشتراك: ${new Date(u.premium_expires_at).toLocaleDateString("ar-SA")}`:"بدون اشتراك نشط"}</p><div class="actions inline-actions">${roleButton}</div></article>`}).join(""):"<div class='empty-state'>لا يوجد مستخدمون.</div>";
  el.subscriptionRequestsList.querySelectorAll("[data-approve-request]").forEach(b=>b.onclick=()=>approveRequest(b.dataset.approveRequest,b.dataset.userId,b.dataset.period,Number(b.dataset.months)));
...
async function approveRequest(requestId,userId,period,months){const{error}=await db.rpc("premium_admin_activate_subscription_months",{p_user_id:userId,p_months:months,p_billing_period:period,p_request_id:requestId,p_admin_note:`تم تفعيل الاشتراك ${period==="monthly"?"الشهري":"السنوي"} من لوحة الإدارة`});if(error)return showDatabaseStatus(error.message,true);showDatabaseStatus(`تم تفعيل Premium لمدة ${months===1?"شهر":"سنة"}.`);await loadAdminData();}
async function rejectRequest(requestId){const{error}=await db.rpc("premium_admin_reject_request",{p_request_id:requestId,p_admin_note:"تم رفض الطلب"});if(error)return showDatabaseStatus(error.message,true);showDatabaseStatus("تم رفض الطلب.");await loadAdminData();}
...
el.paymentConfirmCheckbox?.addEventListener("change",()=>{el.confirmPremiumRequestButton.disabled=!el.paymentConfirmCheckbox.checked;});
