...
const LABELS={results_analysis:"باقة تحليل النتائج",guidance_records:"باقة السجلات الرقمية",all_access:"الباقة الشاملة",presentations:"منصة العروض التقديمية",counselor_plan:"منصة خطة الموجه الطلابي",achievement_reports:"منصة تقارير الإنجاز",messages_library:"مكتبة رسائل أولياء الأمور",certificates_gift:"شهادات التقدير — هدية"};
const PLATFORMS=[
...
function hasGiftCertificatesAccess(){return isAdmin()||activeEntitlements().some(e=>e.product_code==='all_access'||e.billing_period==='yearly')||activePromoAccesses().some(x=>x.product_code==='all_access')}
function hasAccess(code){if(code==='messages_library')return hasAnnualPackage();if(code==='certificates_gift')return hasGiftCertificatesAccess();return isAdmin()||hasAllAccess()||activeEntitlements().some(e=>e.product_code===code)||promoCovers(code)}
...
function launchPlatform(code,href){
  if(!hasAccess(code)){showSubscription(code);return}
...
function renderIdentity(){
 ...
 el.accessExplanation.textContent='تظهر المنصات المشمولة في باقتك فقط بحالة مفعّلة، وباقي المنصات تكون غير نشطة حتى طلب الاشتراك.';
 renderPlatforms();
}
...
function renderPlatforms(){
 ...
 if(el.activePlatformGrid)el.activePlatformGrid.innerHTML=activeCards.length?activeCards.map(x=>x.html).join(''):`<div class="empty-platform-state"><strong>لا توجد منصة مفعّلة حاليًا</strong><span>اختر باقة من قسم الاشتراكات لبدء الاستخدام.</span></div>`;
 if(el.lockedPlatformGrid)el.lockedPlatformGrid.innerHTML=inactiveCards.map(x=>x.html).join('');
 if(el.activePlatformCount)el.activePlatformCount.textContent=String(activeCards.length);
 if(el.lockedPlatformCount)el.lockedPlatformCount.textContent=String(inactiveCards.length);
 if(el.activePlatformCountBadge)el.activePlatformCountBadge.textContent=String(activeCards.length);
...
function showSubscription(product){if(product==='messages_library'||product==='certificates_gift'){const current=activeEntitlements().find(e=>e.billing_period==='monthly'&&e.product_code!=='all_access');const target=product==='certificates_gift'?(current?.product_code||'all_access'):(current?.product_code||'all_access');$all('input[name="productCode"]').forEach(x=>x.checked=x.value===target);$all('input[name="billingPeriod"]').forEach(x=>x.checked=x.value==='yearly');}else if(product){$all('input[name="productCode"]').forEach(x=>x.checked=x.value===product)}el.whatsappConfirmed.checked=false;hideStatus(el.subscriptionStatus);updatePlan();if(product==='messages_library')showStatus(el.subscriptionStatus,'مكتبة الرسائل ميزة مفعلة حصريًا لأي باقة سنوية. اختر الترقية السنوية لإتاحتها.',false);if(product==='certificates_gift')showStatus(el.subscriptionStatus,'منصة شهادات التقدير هدية مجانية لأي باقة سنوية أو للباقة الشاملة. اختر الترقية السنوية لإتاحتها.',false);openModal('subscriptionModal')}
