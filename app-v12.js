...
function hasAccess(code){if(code==='messages_library')return hasAnnualPackage();if(code==='certificates_gift')return hasGiftCertificatesAccess();if(code==='smart_calendar')return hasAllAccess();return isAdmin()||hasAllAccess()||activeEntitlements().some(e=>e.product_code===code)||promoCovers(code)}
...
function showSubscription(product){if(product==='smart_calendar'){$all('input[name="productCode"]').forEach(x=>x.checked=x.value==='all_access');}else if(product==='messages_library'||product==='certificates_gift'){const current=activeEntitlements().find(e=>e.billing_period==='monthly'&&e.product_code!=='all_access');const target=current?.product_code||'all_access';$all('input[name="productCode"]').forEach(x=>x.checked=x.value===target);$all('input[name="billingPeriod"]').forEach(x=>x.checked=x.value==='yearly');}else if(product){$all('input[name="productCode"]').forEach(x=>x.checked=x.value===product)}el.whatsappConfirmed.checked=false;hideStatus(el.subscriptionStatus);updatePlan();if(product==='messages_library')showStatus(el.subscriptionStatus,'مكتبة الرسائل ميزة مفعلة حصريًا لأي باقة سنوية. اختر الترقية السنوية لإتاحتها.',false);if(product==='certificates_gift')showStatus(el.subscriptionStatus,'منصة شهادات التقدير هدية مجانية لأي باقة سنوية أو للباقة الشاملة. اختر الترقية السنوية لإتاحتها.',false);if(product==='smart_calendar')showStatus(el.subscriptionStatus,'التقويم الذكي والتنبيهات ميزة حصرية للباقة الشاملة. اختر الشاملة الشهرية أو السنوية لإتاحتها.',false);openModal('subscriptionModal')}
