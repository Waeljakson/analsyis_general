...
const PACKAGE_LABELS={results_analysis:"باقة تحليل النتائج",guidance_records:"باقة السجلات الرقمية",presentations:"باقة العروض التقديمية",counselor_plan:"باقة خطة الموجه الطلابي",program_ideas:"باقة أفكار البرامج",all_access:"الباقة الشاملة"};
...
function renderPlatformSwitcher(){
  ...
  el.platformPlanBadge.innerHTML=`<span class="platform-plan-status${plan.trial?" trial":""}"></span><span class="platform-plan-copy"><span>الباقة الحالية</span><strong>${escapeHtml(plan.label)}</strong><small>${escapeHtml(plan.detail)}</small></span>`;
...
function renderAudienceControls(){
  ...
  el.stageQuickFilters.innerHTML=META.stages.map(item=>`<button class="quick-filter${state.stage===item.value?" active":""}" data-stage="${item.value}" type="button">${escapeHtml(item.label)}</button>`).join("");
  ...
  el.stageChooser.innerHTML=META.stages.map(item=>`<button class="choice-card${state.stage===item.value?" active":""}" data-stage-choice="${item.value}" type="button"><strong>${escapeHtml(item.label)}</strong><small>${escapeHtml(item.detail||item.tone||"")}</small></button>`).join("");
...
async function confirmAudience(){
  ...
  if(!locked){
    el.confirmAudienceButton.disabled=true;el.confirmAudienceButton.textContent="جارٍ تثبيت النوع...";
    try{const{data,error}=await db.rpc("premium_choose_presentation_audience",{p_audience_type:selected});if(error)throw error;state.account=Object.freeze({...data});state.gender=data.presentation_audience_type;toast(`تم تثبيت نوع العروض: ${GENDER_LABELS[state.gender]||state.gender}`)}
    catch(error){toast(audienceTypeError(error),true);renderAudienceControls();return}
  }
...
function updateCustomWhatsApp(){const request=customRequestPayload();const message=["السلام عليكم، أرغب في تصميم عرض تقديمي خاص.",`العنوان: ${request.title||"لم يحدد بعد"}`,`التصنيف: ${CATEGORY_LABELS[request.category]||"برنامج خاص"}`,`المرحلة: ${STAGE_LABELS[request.stage]||request.stage}`,`الجمهور: ${GENDER_LABELS[request.gender]||request.gender}`,`عدد الشرائح المتوقع: ${request.slides}`,`الهدف: ${request.objective||"سيتم توضيحه"}`,`المدرسة: ${state.account?.school_name||"غير محددة"}`,`الاسم: ${state.account?.full_name||state.user?.email||""}`,`ملاحظات: ${request.notes||"لا توجد"}`,"أرجو مراجعة الطلب وتحديد السعر حسب التكلفة."].join("\n");el.customWhatsAppLink.href=`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(message)}`;el.submitCustomRequestButton.disabled=!el.customWhatsappConfirmed.checked||!request.title||!request.objective||request.slides<15||request.slides>40}
function openCustomRequest(){if(!assignedAudienceType()){openModal("audienceModal");return toast("حدد نوع الطلاب وثبّته أولًا.",true)}el.customStage.value=state.stage;el.customGender.value=assignedAudienceType();el.customWhatsappConfirmed.checked=false;hideStatus(el.customRequestStatus);updateCustomWhatsApp();openModal("customRequestModal")}
async function submitCustomRequest(){const request=customRequestPayload();if(!request.title||!request.objective)return showStatus(el.customRequestStatus,"اكتب عنوان البرنامج والهدف المطلوب.",true);if(!el.customWhatsappConfirmed.checked)return showStatus(el.customRequestStatus,"تواصل عبر واتساب ثم فعّل الإقرار.",true);el.submitCustomRequestButton.disabled=true;try{const{error}=await db.rpc("premium_submit_custom_presentation_request",{p_title:request.title,p_category:request.category,p_stage:request.stage,p_audience_gender:request.gender,p_objective:request.objective,p_desired_slides:request.slides,p_delivery_notes:request.notes||null});if(error)throw error;showStatus(el.customRequestStatus,"تم تسجيل الطلب. سيُراجع النطاق ويُحدد السعر عبر واتساب.");toast("تم إرسال طلب التصميم");el.customTitle.value="";el.customObjective.value="";el.customNotes.value="";el.customSlides.value="15";el.customWhatsappConfirmed.checked=false;await loadMyRequests();updateCustomWhatsApp()}catch(error){showStatus(el.customRequestStatus,error.message||"تعذر إرسال الطلب.",true)}finally{updateCustomWhatsApp()}}
...
async function loadMyRequests(){const{data,error}=await db.from("premium_custom_presentation_requests").select("id,title,stage,audience_gender,desired_slides,status,estimated_price_sar,created_at").eq("user_id",state.user.id).order("created_at",{ascending:false});if(error){el.myRequestsList.innerHTML=`<div class="status-box error">${escapeHtml(error.message)}</div>`;return}state.requests=data||[];renderMyRequests()}
function renderMyRequests(){el.myRequestsList.innerHTML=state.requests.length?state.requests.map(request=>`<article class="request-item"><div><h4>${escapeHtml(request.title)}</h4><p>${escapeHtml(STAGE_LABELS[request.stage]||request.stage)} — ${escapeHtml(GENDER_LABELS[request.audience_gender]||request.audience_gender)} — ${request.desired_slides} شرائح — ${formatDate(request.created_at)}${request.estimated_price_sar?` — السعر التقديري ${Number(request.estimated_price_sar).toFixed(0)} ريال`:""}</p></div><span class="request-status ${escapeHtml(request.status)}">${escapeHtml(REQUEST_STATUS_LABELS[request.status]||request.status)}</span></article>`).join(""):'<div class="loading-line">لا توجد طلبات تصميم خاصة حتى الآن.</div>'}
...
async function init(){bind();if(!db)return redirectToPortal("تعذر الاتصال بقاعدة البيانات.");const{data,error}=await db.auth.getSession();if(error)return redirectToPortal("تعذر قراءة جلسة الدخول.");await applySession(data.session);db.auth.onAuthStateChange((event,session)=>{if(event==="SIGNED_OUT"){clearUnifiedLaunches();redirectToPortal("تم تسجيل الخروج.")}else if(session?.user&&session.user.id!==state.user?.id)setTimeout(()=>applySession(session),0)})}
init();
