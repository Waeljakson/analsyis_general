...
const RECORDS = {
  ...
  daily_incident: {
    ...
    studentKey: "student_name", classKey: "class_name", sections: [
      section("بيانات الموقف", [f("student_name","اسم الطالب","text",{span:6,required:true}),f("class_name","الفصل","text",{span:3}),f("incident_date","التاريخ","date",{span:3}),f("incident_category","نوع الموقف","radio",{options:["سلوكي","تعليمي"]}),f("behavior_code","الموقف السلوكي","select",{span:6,options:["",...DAILY_BEHAVIOR_CODES]}),f("education_code","الموقف التعليمي","select",{span:6,options:["",...DAILY_EDUCATION_CODES]}),f("referral_source","مصدر الإحالة","text",{span:6}),f("action_codes","الإجراءات المتخذة","checklist",{options:DAILY_ACTION_CODES}),f("incident_details","تفاصيل الموقف","textarea",{span:12,rows:5}),f("notes","ملاحظات","textarea",{span:12,rows:3})]),
...
function updateRecordIdentity(){
  const name=state.account?.school_name||"اسم المدرسة";
  el.recordSchoolName.textContent=name;
  el.recordCounselorName.textContent=state.account?.full_name||state.user?.email||"—";
...
async function loadArchive(force=false){
  ...
  if(!isPremiumAccess()){
    ...
    renderArchive();updateStats();return;
  }
  if(state.archiveLoaded&&!force){renderArchive();return;}
  ...
  try{
    ...
    if(error)throw error;state.records=data||[];state.archiveLoaded=true;hideBox(el.archiveStatus);renderArchive();updateStats();
...
async function deleteRecord(id){
  ...
  const {error}=await db.from("guidance_digital_records").delete().eq("id",id);if(error)return showToast(error.message,true);state.records=state.records.filter(r=>r.id!==id);renderArchive();updateStats();showToast("تم حذف السجل.");
}
...
function applyAccountUI(){
  const a=state.account||{};const displayName=a.full_name||state.user?.email||"مستخدم";const school=a.school_name||"أضف اسم المدرسة من الإعدادات";const logo=state.pendingSchoolLogo||a.school_logo_data;
...
function bindEvents(){
  ...
  el.archiveList.addEventListener("click",e=>{const edit=e.target.closest("[data-edit-record]");if(edit)return openSavedRecord(edit.dataset.editRecord);const print=e.target.closest("[data-print-saved]");if(print)return openSavedRecord(print.dataset.printSaved,true);const del=e.target.closest("[data-delete-record]");if(del)return deleteRecord(del.dataset.deleteRecord);});
